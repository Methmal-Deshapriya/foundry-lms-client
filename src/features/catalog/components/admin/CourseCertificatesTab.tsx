  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Input
          aria-label="Search certificates by code or student"
          value={q}
          onChange={(event) => {
        ...
        <Table className="table-fixed">
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">
                  {q || statusFilter ? "No certificates match your filters." : "No certificates issued for this course yet."}
                </TableCell>
              </TableRow>
            ) : (
      ...
      {total > 0 && data?.pagination.offset != null ? (
        <OffsetPagination
          id="certificates-page-size"
          total={total}
          offset={data.pagination.offset}
          pageSize={pageSize}
          shownCount={certificates.length}
                    ...
                    <>
                      {detailCertificate.revokedAt ? (
                        <p className="flex justify-between gap-4">
                          ...
                          <span className="font-medium">{format(new Date(detailCertificate.revokedAt), "MMM dd, yyyy")}</span>
                        </p>
                      ) : null}
